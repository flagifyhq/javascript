...
3. Passes `key={user?.id ?? 'anonymous'}` internally, so login / logout / impersonation switches trigger a clean resync without you wiring the key manually.
...
| `publicKey` | `string` | Yes | Client-safe publishable API key |
| `secretKey` | `string` | No | Server-side secret key |
| `options` | `object` | No | All `FlagifyOptions` except `user`, which the wrapper owns |
| `children` | `ReactNode` | Yes | Your application tree |
...
**3. Auth via Zustand / Redux selector.** The user lives in a synchronous store selector. Use `<FlagifyAuthProvider>` with the selector as the hook — no remount dance because the selector is cheap and synchronous.
...
| `FlagifyProviderChildren` | Type | Children type accepted by `FlagifyProvider` — use this to bypass `@types/react` version skew in strict TypeScript consumers |
